...
st.markdown(
    "<h1 class='main-header'>LLM-powered Assistant</h1>",
    unsafe_allow_html=True,
)
...
with col1:
    if st.button(
        ...
        key="btn_scraper",
        type=(
            "primary" if st.session_state.current_page == "Web Scraper" else "secondary"
        ),
    ):
        st.session_state.current_page = "Web Scraper"
        st.rerun()
...
with col2:
    if st.button(
        ...
        type=(
            "primary"
            if st.session_state.current_page == "Subsidy Enquiries"
            else "secondary"
        ),
    ):
...
if st.session_state.current_page == "Web Scraper":
    st.markdown(
        "<h2 class='subheader'>Web Scraper</h2>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<h3 class='subheader'>Step 1: Enter Website URL</h3>",
        unsafe_allow_html=True,
    )
    ...
    with col1:
        url = st.text_input(
            "Website URL",
            placeholder="https://tashif.codes",
            key="scraper_url",
        )
    ...
    if "dom_content" in st.session_state:
        st.markdown(
            "<h3 class='subheader'>Step 2: Extract Information</h3>",
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        "<h2 class='subheader'>Solar Subsidy Enquiries</h2>",
        unsafe_allow_html=True,
    )
    ...
    def read_subsidy_info():
        subsidy_file_path = os.path.join(
        ...
        try:
            ...
            st.error(
                "Subsidy information file not found. Please create a 'subsidy_info.md' file."
            )
            return None
    ...
    st.markdown(
        "<h3 class='subheader'>Ask a Question</h3>",
        unsafe_allow_html=True,
    )
    query = st.text_area(
    ...
    if st.button(
        ...
        if query and "subsidy_info" in st.session_state:
            ...
            if "subsidy_info" not in st.session_state:
                st.warning(
                    "⚠️ Subsidy information is not available. Please check the subsidy_info.md file."
                )
