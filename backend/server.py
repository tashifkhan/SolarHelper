...
async def get_solar_recommendation(request: RecommendationRequest):
    ...
    to ask an LLM for a tailored solar setup recommendation. It also checks
...
10. **Strictly format your entire response as a single JSON object matching the following structure:**
```json
{json_output_format}
```
Do not include any text before or after the JSON object. Ensure the JSON is valid.
"""  # Modified instruction 9 slightly to guide LLM better
        ...
        try:
            ...
            recommendation = SolarRecommendation.model_validate_json(cleaned_llm_output.strip())
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to parse the recommendation from the AI model. Error: {parse_error}. Raw output: {llm_output}"
            )
        ...
        try:
            ...
            # Proceed with check only if all values could be parsed
        ...
        # Re-raise HTTPExceptions directly (like the budget check one)
        raise http_exc
    except Exception as e:
